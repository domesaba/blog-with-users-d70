...
def admin_only(func):
    """restrict routes to admin user with id 1"""
    @wraps(func)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("user already exist, please log in instead")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("user not registered. Please try again")
            return redirect(url_for('login'))
